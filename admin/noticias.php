...
//Elemento Nuevo
    ...
    <fieldset>
      ...
      <?php echo draw_select("Categoría: ",'not_cat',$cat_not,'','table_horiz','','req'); ?>
...
//Validación de Elemento Nuevo
    case 'vn':
      ...
      $not_cat = str_input($_POST['not_cat']);
      ...
      if (empty($not_nombre) OR empty($not_resena) OR empty($not_cat) OR empty($not_pub) OR empty($not_dest)){
        ...
        if (empty($not_nombre)) $validation .= "Debe escribir un nombre para la noticia.<br />";
        if (empty($not_cat)) $validation .= "Debe seleccionar una categoría para la noticia.<br />";
        if (empty($not_pub)) $validation .= "Debe seleccionar si la noticia será publicada o no.<br />";
        if (empty($not_dest)) $validation .= "Debe seleccionar si la noticia será destacada o no.<br />";
        if (empty($not_resena)) $validation .= "Debe incorporar una reseña para la noticia. <br />";
...
?>
    ...
    <fieldset>
      ...
      <?php echo draw_select("Categoría: ",'not_cat',$cat_not,$not_cat,'table_horiz','','req'); ?>
...
//Modificación
    ...
    $datos = mysql_query("SELECT * FROM ".$tabla." WHERE not_id=".$id."") or die(mysql_error());
      ...
      $not_cat = str_output(mysql_result($datos,0,'not_cat'));
...
?>
      <fieldset>
        ...
        <div class="box_info">
          <p>¿Está seguro/a que desea eliminar esta noticia?</p>
        </div>
          <div align="center">
...
<?php
      ...
      for($j=0; $j<$num_rows; ++$j) {
          ...
          foreach ($cat_not as $key => $value) {if ($key == $not_cat) {$not_cat_name = "$value";}}
...
?>
          <tr>
            ...
            <td class="<?php echo $celda; ?>">
              ...
              <p><strong>Categoría:</strong> <?php echo $not_cat_name; ?></p>
